"""
NumPy based implemenation of max-heap data structure.
"""
    ...
    def extract_max(self):
        ...
        while self.heap[key_idx] <= self.heap[c1_idx] or self.heap[key_idx] <= self.heap[c2_idx]:
            if self.heap[c1_idx] >= self.heap[c2_idx]:
                greater_child_idx = c1_idx
            else:
                greater_child_idx = c2_idx
            temp = self.heap[greater_child_idx]
            self.heap[greater_child_idx] = self.heap[key_idx]
            self.heap[key_idx] = temp

            key_idx = greater_child_idx
            c1_idx, c2_idx = self.get_children_idx(key_idx)
